    ...
    <div style={{paddingTop:2}}>
      ...
      <div style={{background:'#fff',border:`1px solid ${C.l100}`,padding:'18px 20px',marginBottom:2}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          <div>
            <div style={{fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',color:'#F3A2BE',marginBottom:4}}>🎈 Акцентни балони (латекс)</div>
            <div style={{fontSize:10,color:C.gray}}>добавят се към букетите без да влияят на дължината</div>
          </div>
          <button style={pill(false,'#F3A2BE')} onClick={()=>set('accents',[...accents,{hex:'#e8c0dc',name:'',sizeInch:5,sizeCm:13,customSize:false,perAccent:3,customQty:false,price:0,distribution:'all',customCount:false,customCountVal:1}])}>
            + Добави акцент
          </button>
        </div>
          ...
          return (
            <div key={ai} style={{display:'grid',gridTemplateColumns:'1fr 1.4fr 1.2fr',gap:2,marginBottom:4}}>
              ...
              <div style={{background:C.l50,padding:'12px',borderLeft:`3px solid ${ac.hex||'#9b7dd4'}`}}>
                ...
                <div style={{fontSize:9,color:'#81BFB7',marginTop:3}}>⚠️ Себестойност, не продажна цена</div>
                {clusters>0 && <div style={{marginTop:8,fontSize:11,color:'#F3A2BE',fontWeight:600}}>{cnt.acClusters} × {ac.perAccent} = {cnt.nr} бр · €{(cnt.cost||0).toFixed(2)}</div>}
              </div>
              {/* размер */}
              <div style={{background:C.l50,padding:'12px'}}>
                <div style={{fontSize:10,fontWeight:600,color:'#F3A2BE',textTransform:'uppercase',marginBottom:8}}>Размер</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:4}}>
                  {[{inch:4,cm:10},{inch:5,cm:13},{inch:6,cm:15},{inch:18,cm:46},{inch:24,cm:61},{inch:36,cm:91}].map(sz=>(
                    <button key={sz.inch} style={{...pillLight(ac.sizeInch===sz.inch&&!ac.customSize,'#F3A2BE'),padding:'7px 3px',fontSize:10,textAlign:'center'}}
                      onClick={()=>updAccent(ai,{sizeInch:sz.inch,sizeCm:sz.cm,customSize:false})}>
                      {sz.inch}"<br/><span style={{fontSize:9}}>({sz.cm}см)</span>
                    </button>
                  ))}
                  <button style={{...pillLight(ac.customSize,'#F3A2BE'),padding:'7px 3px',fontSize:10}} onClick={()=>updAccent(ai,{customSize:true})}>друго</button>
                </div>
              </div>
              {/* брой + добави към */}
              <div style={{background:C.l50,padding:'12px'}}>
                <div style={{fontSize:10,fontWeight:600,color:'#F3A2BE',textTransform:'uppercase',marginBottom:8}}>В букет</div>
                <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:10}}>
                  {[1,2,3,4,5].map(n=>(
                    <button key={n} style={{...pillLight(ac.perAccent===n&&!ac.customQty,'#F3A2BE'),padding:'7px 10px',fontSize:12,fontWeight:600}}
                      onClick={()=>updAccent(ai,{perAccent:n,customQty:false})}>{n}</button>
                  ))}
                  <button style={{...pillLight(ac.customQty,'#F3A2BE'),padding:'7px 8px',fontSize:11}}
                    onClick={()=>updAccent(ai,{customQty:true})}>+/-</button>
                </div>
                {ac.customQty && (
                  <div style={{display:'flex',gap:6,alignItems:'center',marginBottom:10}}>
                    <button style={{...pillLight(false,'#F3A2BE'),padding:'5px 10px'}} onClick={()=>updAccent(ai,{perAccent:Math.max(1,(ac.perAccent||1)-1)})}>−</button>
                    ...
                    <button style={{...pillLight(false,'#F3A2BE'),padding:'5px 10px'}} onClick={()=>updAccent(ai,{perAccent:(ac.perAccent||1)+1})}>+</button>
                  </div>
                )}
                <div style={{fontSize:10,fontWeight:600,color:'#F3A2BE',marginBottom:6}}>Добави към</div>
                <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                  <button style={{...pillLight(ac.distribution==='all'&&!ac.customCount,'#F3A2BE'),fontSize:10,padding:'5px 8px'}}
                    onClick={()=>updAccent(ai,{distribution:'all',customCount:false})}>Всеки ({clusters})</button>
                  {colorEntries.slice(0,numColors).map((ce,ci)=>(
                    <button key={ci} style={{...pillLight(ac.distribution===String(ci)&&!ac.customCount,ce.hex||'#F3A2BE'),fontSize:10,padding:'5px 8px'}}
                      onClick={()=>updAccent(ai,{distribution:String(ci),customCount:false})}>
                      {ce.name||`Цвят ${ci+1}`} ({clustersPerColor[ci]||0})
                    </button>
                  ))}
                  <button style={{...pillLight(ac.customCount,'#F3A2BE'),fontSize:10,padding:'5px 8px'}}
                ...
                {ac.customCount && (
                  <div style={{display:'flex',alignItems:'center',gap:8,marginTop:8,background:'#fff',padding:'6px 8px'}}>
                    <button style={{...pillLight(false,'#F3A2BE'),padding:'4px 8px'}} onClick={()=>updAccent(ai,{customCountVal:Math.max(1,(ac.customCountVal||1)-1)})}>−</button>
                    ...
                    <button style={{...pillLight(false,'#F3A2BE'),padding:'4px 8px'}} onClick={()=>updAccent(ai,{customCountVal:(ac.customCountVal||1)+1})}>+</button>
      ...
      <div style={{background:'#fff',border:`1px solid ${C.l100}`,padding:'18px 20px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          <div>
            <div style={{fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',color:'#81BFB7',marginBottom:4}}>✨ Фолио балони</div>
            <div style={{fontSize:10,color:C.gray}}>надуват се внимателно със сламка — добави ги за точна времева оценка</div>
          </div>
          <button style={pill(false,'#81BFB7')} onClick={()=>set('foilBalloons',[...foilBalloons,{name:'',inch:18,cm:46,label:'18" стандартен',customSize:false,qty:1,price:0,timeMin:2}])}>
            + Добави фолио
          </button>
        </div>
        {foilBalloons.length===0 && <div style={{fontSize:11,color:C.gray,padding:'10px',background:'#F0F9F8',textAlign:'center'}}>Няма добавени фолио балони</div>}
        {foilBalloons.map((fb,fi)=>(
          <div key={fi} style={{display:'grid',gridTemplateColumns:'1fr 1.4fr 1.2fr',gap:2,marginBottom:4}}>
            <div style={{background:'#F0F9F8',padding:'12px',borderLeft:'3px solid #81BFB7'}}>
              <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:8}}>
              ...
              <div style={{marginTop:8,fontSize:11,color:'#81BFB7',fontWeight:600}}>{fb.qty} бр · {(fb.qty*(fb.timeMin||2)).toFixed(0)} мин · €{(fb.qty*(fb.price||0)).toFixed(2)}</div>
            </div>
            <div style={{background:'#F0F9F8',padding:'12px'}}>
              <div style={{fontSize:10,fontWeight:600,color:'#81BFB7',textTransform:'uppercase',marginBottom:8}}>Размер</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}}>
                {FOIL_SIZES.map((sz,si)=>(
                  <button key={si} style={{...pillLight(fb.inch===sz.inch&&fb.label===sz.label,'#81BFB7'),padding:'7px 4px',fontSize:10,textAlign:'center'}}
            ...
            <div style={{background:'#F0F9F8',padding:'12px'}}>
              <div style={{fontSize:10,fontWeight:600,color:'#81BFB7',textTransform:'uppercase',marginBottom:8}}>Надуване (мин/бр)</div>
              ...
              <div style={{marginTop:8,fontSize:10,color:'#81BFB7'}}>общо: {(fb.qty*(fb.timeMin||2)).toFixed(0)} мин</div>
            </div>
          </div>
        ))}
      </div>
